// DeepSeek real-detector-size latency probe.
...
const tries = [
  { label: 'deepseek-v4-flash max=4096', payload: { model: 'deepseek-v4-flash', messages: [{ role: 'system', content: system }, { role: 'user', content: chat }], max_tokens: 4096 } },
  { label: 'deepseek-v4-pro max=4096', payload: { model: 'deepseek-v4-pro', messages: [{ role: 'system', content: system }, { role: 'user', content: chat }], max_tokens: 4096 } },
];
...
for (const t of tries) {
  for (let i = 0; i < 3; i++) {
    ...
    const isJson = text.trim().startsWith('{') || isFenced;
    console.log(`  [${t.label}]  run ${i + 1}  ${ms}ms  finish=${finish}  prompt=${usage.prompt_tokens}  completion=${usage.completion_tokens}  reasoning=${usage.completion_tokens_details?.reasoning_tokens ?? 'n/a'}  has_think=${hasThink}  is_fenced=${isFenced}  preview=${JSON.stringify(text.slice(0, 80))}`);
  }
  console.log();
}
