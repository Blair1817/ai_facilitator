...
export function FinalDecision() {
  ...
  const [choice, setChoice] = useState(() => player.round.get("groupFinalChoice") ?? "");
  const [confidence, setConfidence] = useState(() => player.round.get("groupChoiceConfidence") ?? null);
  const submitting = useRef(false);
  const updateChoice = (value) => {
    setChoice(value);
    player.round.set("groupFinalChoice", value);
    if (value === NO_GROUP_FINAL_DECISION_OPTION.id) {
      setConfidence(null);
      player.round.set("groupChoiceConfidence", null);
    }
  };
  const updateConfidence = (value) => { setConfidence(value); player.round.set("groupChoiceConfidence", value); };
