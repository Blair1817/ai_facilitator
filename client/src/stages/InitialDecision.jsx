...
export function InitialDecision() {
  ...
  const choiceDraftKey = draftKey({ playerId: player.id, roundId: round?.id, form: "initialDecision", field: "choice" });
  const confidenceDraftKey = draftKey({ playerId: player.id, roundId: round?.id, form: "initialDecision", field: "confidence" });
  const [choice, setChoice] = usePersistentDraft(choiceDraftKey, player.round.get("initialChoice") ?? "");
  const [confidence, setConfidence] = usePersistentDraft(confidenceDraftKey, player.round.get("initialConfidence") ?? null);
  const submitting = useRef(false);

  const updateChoice = setChoice;
