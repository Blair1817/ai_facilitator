...
# D-014: the research-data export server now runs inside the Empirica
# callbacks process (server/src/index.js starts it on Tajriba "ready").
# This keeps the bundle count at one and avoids a second Node process
# the operator has to supervise. To disable, set
# DELIBRA_DISABLE_EXPORT_SERVER=1 in the environment; the callbacks
# process then logs a clear message and continues.
